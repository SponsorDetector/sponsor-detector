...
var SponsorDetector = new function() {
  ...
  this.apply = function(conf, domain) {
    ...
    if (conf.author && conf.author.extractor && conf.author.extractor.name) {
      var author = ExtractorServices.services[conf.author.extractor.name].call
        .apply(null, conf.author.extractor.params);
      if (author) {
    ...
    if (conf.sponsor && conf.sponsor.extractor) {
      var sponsor = ExtractorServices.services[conf.sponsor.extractor.name]
        .call.apply(null, conf.sponsor.extractor.params);
      if (sponsor) {
    ...
    if (conf.sponsor && conf.sponsor.detector && conf.sponsor.detector.name) {
      var sponsorised = DetectorServices.services[conf.sponsor.detector.name]
        .call.apply(null, conf.sponsor.detector.params);
      if (sponsorised) {
